...
const SafeToSpendGauge = ({ pct = 0, size = 220 }) => {
    ...
    const color = clamped >= 100 ? 'var(--destructive)' : clamped >= 80 ? 'var(--warning)' : 'var(--success)';
    ...
    return (
        <svg
            ...
            aria-valuemin={0}
            aria-valuemax={100}
            aria-label={`${Math.round(clamped)}% of today's safe-to-spend budget used`}
        >
            <path
                d={trackPath}
                fill="none"
